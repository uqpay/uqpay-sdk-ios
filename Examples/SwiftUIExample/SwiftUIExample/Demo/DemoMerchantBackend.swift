...
actor DemoMerchantBackend: UqpayCredentialProvider {
    ...
    enum DemoError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notConfigured:
                return DemoConfiguration.setupInstructions
    ...
    private lazy var apiClient = UqpayHTTPClient(
        ...
        clientID: DemoConfiguration.clientID,
        credentials: self
    )
    ...
    func authToken() async throws -> String {
        guard DemoConfiguration.isConfigured else { throw DemoError.notConfigured }

        if let cachedToken, let tokenExpiry, tokenExpiry > Date() {
            return cachedToken
        }

        let url = DemoConfiguration.environment.baseURL
            .appendingPathComponent("api/v1/connect/token")

        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue(DemoConfiguration.clientID, forHTTPHeaderField: "x-client-id")
        request.setValue(DemoConfiguration.apiKey, forHTTPHeaderField: "x-api-key")
